/**********************************************************
# simulating data from an ECCC-GARCH(1,1) and computing an 
# ECCC-GARCH(1,1) volatility.

dyn.load("R_eccc_sim.dll")
eccc.sim = function(nobs, a, A, B, R, nu = Inf, cut=1000){
  nobs = nobs+cut
  Id = diag(length(a))
  inih = solve(Id-A-B)%*%a
  sim = .Call("eccc_sim", nobs, a, A, B, R, inih, nu)
  h = sim[[1]]; eps = sim[[2]]
  list(h = h[(cut+1):(nobs),], eps = eps[(cut+1):(nobs),])
}

nobs = 10000; cut=1000
a = c(0.003, 0.005); A = diag(c(0.2,0.3)); B = diag(c(0.79, 0.6))
R = matrix(c(1.0, 0.4, 0.4, 1.0),2,2)
nu = 30
temp = eccc.sim(nobs,a, A, B, R, nu = nu, cut)
